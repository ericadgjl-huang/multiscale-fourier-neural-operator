"""
unetpp_baseline.py
純 2D UNet++ baseline（無 FNO）。
UNet++ 與 UNet 的核心差異：用「nested dense skip pathway」取代單一跳接，
讓不同深度特徵漸進融合，理論上對細節保留與多尺度特徵更友善。
...
################################################################
# DoubleConv（與 unet_baseline.py 相同）
################################################################
...
################################################################
# UNetPlusPlus2DRollout
#
# 節點命名 x_{i,j}：i = 深度層 (0~3)，j = 跳接路徑階段 (0~3-i)
# 結構：
#   x00 ─ x01 ─ x02 ─ x03 ─→ output
#    │     ↑     ↑     ↑
#   x10 ─ x11 ─ x12
#    │     ↑     ↑
#   x20 ─ x21
#    │     ↑
#   x30
# 規則：x_{i,j} = DoubleConv(concat([x_{i,k} for k<j] + [up(x_{i+1,j-1})]))
################################################################
class UNetPlusPlus2DRollout(nn.Module):
    def __init__(self, in_channels=10, out_channels=4, base=32, lon_pad=4):
        ...
        ch = [base, base * 2, base * 4, base * 8]   # 32, 64, 128, 256
    ...
    def forward(self, x):
        ...
        # j=3 nested skip：最終輸出層
        x03 = self.x03(torch.cat([x00, x01, x02, self._up_match(x12, x00)], dim=1))

        out = self.outc(x03)
        out = out[..., self.lon_pad:-self.lon_pad]     # 去除經度 padding
...
# 訓練與評估
################################################################
model = UNetPlusPlus2DRollout(in_channels=10, out_channels=4,
                              base=base_width, lon_pad=lon_pad).cuda()
...
plt.title(f'Learning Curve — {display_name} ({rollout_steps * 6 // 24}-Day Forecast)', fontsize=13)
