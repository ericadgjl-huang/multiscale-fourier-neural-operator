...
| 訓練跑完、存了權重/log，但**畫圖時 exit 127、無 traceback** | matplotlib 原生 DLL 與 pillow 衝突（Windows 致命例外 `0xc06d007f`）。修：`pip install --force-reinstall --no-cache-dir matplotlib==3.8.0`（新環境已改從 pip 裝，不會再遇到） |
| `.nc` 開不起來 `file signature not found` | 該檔其實是 CDS 回傳的 ZIP（檔頭 `PK`）。先 `python verify_nc.py` 掃、再 `python fix_zip_nc.py` 解壓合併（重下無效，只會再拿到 zip） |
