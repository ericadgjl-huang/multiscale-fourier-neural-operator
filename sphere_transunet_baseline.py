"""
sphere_transunet_baseline.py
純球面 TransUNet baseline：SHT-based encoder/decoder + Transformer bottleneck。
跟 transunet_baseline.py 平行，只是 encoder/decoder 換成 SHT。

對標：transunet_baseline.py (planar)
共用模組：sphere_blocks.py

架構：
    inc:        SHTDoubleConv(10, 32, 33×64, m=8)       x1
    down1:      SHTDown to (16×32, m=4)                  x2
    down2:      SHTDown to (8×16, m=2)                   x3 (bottleneck)
    bottleneck: TransformerBottleneck (4 layers, d=128)
    up1:        SHTUp 8×16 -> 16×32 (m=4)
    up2:        SHTUp 16×32 -> 33×64 (m=8)
    outc:       Conv2d(32, 4, 1)
"""
import os
import json
...
from sphere_blocks import SHTDoubleConv, SHTDown, SHTUp, TransformerBottleneck
...
class SphereTransUNet2DRollout(nn.Module):
    def __init__(self, in_channels=10, out_channels=4, base_width=32,
        ...
        # Transformer bottleneck（在 8×16 = 128 tokens 上做 self-attention）
        # max_tokens 設 200，給未來不同網格留 buffer
        self.bottleneck = TransformerBottleneck(
            channels   = base_width * 4,
            max_tokens = 200,
...
################################################################
# 實驗設定（PR 6：sphere_transunet 純球面）
################################################################
base_experiment_name = 'sphere_transunet'
display_name         = 'Sphere TransUNet (SHT encoder/decoder + Transformer bottleneck, no FNO)'
SEED                 = 0
...
# 訓練與評估
################################################################
model = SphereTransUNet2DRollout(
    in_channels=10, out_channels=4,
    base_width=base_width, nlat=33, nlon=64,
    modes=modes_per_level,
    trans_layers=trans_layers, trans_heads=trans_heads, trans_dropout=trans_dropout,
).cuda()
...
print(f" 解析度：33×64 → 16×32 → 8×16（bottleneck transformer）→ 16×32 → 33×64")
...
config_snapshot = {
    ...
    'arch_family':          'pure_spherical_transunet',
    'seed':                 SEED,
