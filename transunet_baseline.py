"""
transunet_baseline.py
TransUNet baseline：CNN encoder → Transformer bottleneck → CNN decoder（with skip connections）。
與 unet_baseline.py 共用 ERA5RolloutDataset、T-BPTT、視覺化邏輯，僅模型不同。

架構（base_width=32, lon_pad=4 → lat=33, lon=72 after pad）：
    inc           DoubleConv(10, 32)         → (B, 32,  33, 72)   x1
    down1         MaxPool + DoubleConv(64)   → (B, 64,  16, 36)   x2
    down2         MaxPool + DoubleConv(128)  → (B, 128, 8,  18)   x3 (= bottleneck)
    bottleneck    Transformer (4 layers, d=128, h=4, FFN=512)
    up1           Bilinear + DoubleConv      → (B, 64,  16, 36)
    up2           Bilinear + DoubleConv      → (B, 32,  33, 72)
    outc          Conv2d(32, 4, 1)           → (B, 4,   33, 72)

Reference: Chen et al. 2021, "TransUNet: Transformers Make Strong Encoders for Medical Image Segmentation"
"""
import os
import json
import csv
...
################################################################
# Building blocks（與 unet_baseline.py 相同）
################################################################
...
################################################################
# TransformerBottleneck：在 (B, C, H, W) 的 spatial map 上做 self-attention
################################################################
class TransformerBottleneck(nn.Module):
    """
    把 (B, C, H, W) flatten 成 (B, H*W, C) tokens 餵給 transformer，
    再 reshape 回 (B, C, H, W)。
    使用 pre-norm 架構（norm_first=True）以提升訓練穩定度。
    """
    ...
    def forward(self, x):
        ...
        x = x + self.pos_emb[:, :n_tokens, :]         # 加位置編碼
        x = self.transformer(x)                       # self-attention 混合所有空間位置
        x = self.norm_out(x)
        x = x.transpose(1, 2).reshape(B, C, H, W)     # 還原回 spatial map
        return x


################################################################
# TransUNet2DRollout：適配 ERA5 (33×64) + 經度 circular padding
################################################################
class TransUNet2DRollout(nn.Module):
    def __init__(self, in_channels=10, out_channels=4, base_width=32, lon_pad=4,
        ...
        lon_pad      = 經度方向 circular padding 量
        trans_*      = transformer 超參
        """
        super().__init__()
        self.lon_pad = lon_pad

        # CNN encoder（與 unet_baseline 同樣比例）
        ...
        # 預設 ERA5 (33, 64) + lon_pad=4 → lat=33, lon=72
        # 經 2 次 MaxPool2d(2) 後：lat=8, lon=18 → 144 tokens
        # 留 max_tokens=200 給未來不同 padding 設定的彈性
        self.bottleneck = TransformerBottleneck(
            channels   = base_width * 4,    # 128
            max_tokens = 200,
        ...
        # CNN decoder（mirror encoder）
        self.up1  = Up(base_width * 4 + base_width * 2, base_width * 2)
        self.up2  = Up(base_width * 2 + base_width,     base_width)
    ...
    def forward(self, x):
        ...
        x1 = self.inc(x)        # (B, 32, 33, 72)
        x2 = self.down1(x1)     # (B, 64, 16, 36)
        x3 = self.down2(x2)     # (B, 128, 8, 18)

        x3 = self.bottleneck(x3)                     # transformer self-attention

        x = self.up1(x3, x2)    # (B, 64, 16, 36)
        x = self.up2(x, x1)     # (B, 32, 33, 72)
        x = self.outc(x)        # (B, 4, 33, 72)
        x = x[..., self.lon_pad:-self.lon_pad]       # → (B, 4, 33, 64)
...
print(f" Transformer：{trans_layers} layers × {trans_heads} heads, dropout={trans_dropout}")
...
config_snapshot = {
    ...
    'arch_family':          'cnn_transformer_hybrid',
    'seed':                 SEED,
    'base_width':           base_width,
    'lon_pad':              lon_pad,
    'trans_layers':         trans_layers,
