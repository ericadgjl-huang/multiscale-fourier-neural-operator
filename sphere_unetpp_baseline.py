"""
sphere_unetpp_baseline.py
純球面 UNet++ baseline：UNet++ 的 nested dense skip pathway + 所有 conv 用 SHT 取代。

對標：unetpp_baseline.py (planar)
共用模組：sphere_blocks.py

3 層階層（base=32, modes=[8,4,2]）：
    x00 ─ x01 ─ x02 ─→ output
     │     ↑     ↑
    x10 ─ x11
     │     ↑
...
from sphere_blocks import SHTDoubleConv
...
################################################################
# SphereUNetPlusPlus2DRollout：3 層 nested SHT-based UNet++
#
# 節點命名 x_{i,j}：i = 深度層 (0..2)，j = 跳接路徑階段 (0..2-i)
# Backbone（j=0）：x00 -> x10 -> x20
# Nested skip：每個 x_{i,j} = SHTDoubleConv(concat 所有同層前面節點 + 下一層 up)
################################################################
class SphereUNetPlusPlus2DRollout(nn.Module):
    def __init__(self, in_channels=10, out_channels=4, base=32,
        ...
        super().__init__()
        m0, m1, m2 = modes
        ch = [base, base * 2, base * 4]   # 32, 64, 128

        self.pool = nn.MaxPool2d(2)

        # Backbone (j=0)：標準 encoder
        ...
        # j=2：吃 2 個同層 + 1 個下層 up
        self.x02 = SHTDoubleConv(ch[0] * 2 + ch[1], ch[0],
    ...
    def _up_match(x_low, x_high):
        """Bilinear upsample x_low 至 x_high 的空間尺寸（避免奇數對不齊）"""
    ...
    def forward(self, x):
        ...
        # Backbone
        x00 = self.x00(x)                              # (B, 32, 33, 64)
        x10 = self.x10(self.pool(x00))                 # (B, 64, 16, 32)
        x20 = self.x20(self.pool(x10))                 # (B, 128, 8, 16)
        ...
        out = self.outc(x02)                           # (B, 4, 33, 64)
        return out.permute(0, 2, 3, 1)                 # (B, 33, 64, 4)
...
################################################################
# 實驗設定（PR 6：sphere_unetpp 純球面）
################################################################
base_experiment_name = 'sphere_unetpp'
display_name         = 'Sphere UNet++ (Pure SHT-based Nested UNet, no FNO scaffold)'
SEED                 = 0
...
# 訓練與評估
################################################################
model = SphereUNetPlusPlus2DRollout(
    in_channels=10, out_channels=4,
    base=base_width, nlat=33, nlon=64,
...
print(f" 解析度：33×64 → 16×32 → 8×16（3 層 nested）")
...
config_snapshot = {
    ...
    'arch_family':          'pure_spherical_unetpp',
    'seed':                 SEED,
